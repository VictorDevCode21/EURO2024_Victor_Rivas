...
class Match:
    def __init__(self,id: int, number: int, home_team: List[Team] , away_team: List[Team], date_time: date, group: str, stadium: List[Stadium]):
        self.id = id 
        self.number = number
        self.group = group
    ...
    def get_info(self):
        return (f"The match {self.number} is: {self.home_team.get_info()} vs {self.away_team.get_info()} at {self.stadium.get_info()} on {self.date_time}")
